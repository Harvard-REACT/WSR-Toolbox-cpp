                              ...
                              std::unordered_map<std::string, std::string> tx_csi_file)
{
    ...
    for (int num_tx = 0; num_tx < mac_id_tx.size(); num_tx++)
    {
        ...
        else
        {
            if (__FLAG_debug)
            {
                std::cout << "log-debug [test_csi_data (forward-reverse channel)]: CSI_packets_used = " << csi_timestamp_all.shape() << std::endl;
                std::cout << "log-debug [test_csi_data (forward-reverse channel)]: h_list size  = " << h_list_all.shape() << std::endl;
            }
